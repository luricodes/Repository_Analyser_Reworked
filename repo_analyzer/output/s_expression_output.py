# repo_analyzer/output/s_expression_output.py
...
def format_file_entry(name: str, data: Dict[str, Any], indent: str = "") -> str:
    """
...
def format_directory(data: Dict[str, Any], indent: str = "") -> str:
    """
    ...
    for name, value in sorted(data.items()):
        if isinstance(value, dict):
            if "type" in value:  # File entry
                result.append(format_file_entry(name, value, next_indent))
            else:  # Directory entry
                result.append(f"{indent}(directory {format_atom(name, 'name')}")
                subdir = format_directory(value, next_indent)
                if subdir:
...
def output_to_sexp(data: Dict[str, Any], output_file: str) -> None:
    """
    ...
    try:
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            formatted_structure = format_directory(structure, "    ")
            f.write(formatted_structure)
            f.write("\n  )\n")
            
            # Write summary if present
            if "summary" in data:
                formatted_summary = format_summary(data["summary"], "  ")
